...
def main():
    parser = argparse.ArgumentParser(description="Run Ablation Studies for RankKV")
    ...
    if args.ablation_type == 'random':
        ...
        # Shuffle the budget values but assign to layers randomly
        random.seed(42) # Fixed seed for reproducibility
        random.shuffle(final_budgets)
    elif args.ablation_type == 'inverse':
        print("Applying Inverse Rank Logic...")
        # Sort budgets: RankKV assigns higher budgets to more important layers (usually higher rank value means more important?)
        # Let's check allocate_budget_proportional logic.
        # Usually: higher score -> higher budget.
        # If we reverse the budget list (assuming layers are sorted by rank?), no, standard_budgets is a list where index=layer_idx.
        # We need to see which layers got high budgets and swap them with low budgets.
        # The simplest way is to sort the budget values and assign the smallest values to the layers that got the largest values.
    ...
    for ds_name in datasets:
        ...
        snapkv_utils.LAYER_BUDGETS = {i: b for i, b in enumerate(final_budgets)}
