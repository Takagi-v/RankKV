...
def measure_metrics(model, input_ids, generate_len=50):
    ...
    # RankKV-Pyramid uses LAYER_BUDGETS
    ...
    if hasattr(pyramidkv_utils, 'DISABLE_COMPRESSION') and pyramidkv_utils.DISABLE_COMPRESSION:
        ...
        # Fallback to model.config.max_capacity_prompt if not set (unlikely for RankKV run)
        avg_budget = getattr(model.config, 'max_capacity_prompt', 128)
...
def main():
    parser = argparse.ArgumentParser(description="Run RankKV-Pyramid Benchmark")
    ...
    # STRICT VARIABLE CONTROL:
    # Match Original PyramidKV's actual total budget exactly.
    # Original PyramidKV formula results in slightly higher budget than target_avg due to linear step logic.
    def get_original_pyramid_total_budget(num_layers=32, avg_budget=128, window_size=32, beta=20):
        min_num = (avg_budget - window_size) // beta
        max_num = (avg_budget - window_size) * 2 - min_num
        steps = (max_num - min_num) / num_layers
        budgets = [int(max_num - i * steps) + window_size for i in range(num_layers)]
        return sum(budgets)

    target_avg_budget = args.budget
    # total_budget_sum = target_avg_budget * len(ranks) # Old logic: 4096
    total_budget_sum = get_original_pyramid_total_budget(len(ranks), target_avg_budget) # New logic: ~4176
    
    rankkv_budgets = allocate_budget_proportional(ranks, total_budget_sum, min_budget=32)
    print(f"RankKV Budgets (Total {total_budget_sum}, Avg {total_budget_sum/len(ranks):.2f}): {rankkv_budgets}")
    ...
    for ds_name in datasets:
        ...
        pyramidkv_utils.LAYER_BUDGETS.clear() # Should be ignored due to DISABLE_COMPRESSION, but good practice
        ...
        # --- Comparison ---
        print("\n--- Comparison (RankKV vs Standard) ---")
        print(f"{'Metric':<15} | {'RankKV':<15} | {'Standard':<15} | {'Diff':<10}")
        print("-" * 60)
