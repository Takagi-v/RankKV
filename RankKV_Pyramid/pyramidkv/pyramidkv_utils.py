...
MAX_CAPACITY_PROMPT = 256 + 64 # Default value

class PyramidKVCluster():
    def __init__(self, num_hidden_layers = 32, window_size = 64, max_capacity_prompt = None, kernel_size = 5, pooling = 'avgpool', beta = 20, num_layers = 80, layer_idx=None):
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups):
        ...
        max_capacity_prompt = int(max_num - self.layer_idx * steps)
        
        # RankKV Integration: Override budget if provided
        if self.layer_idx in LAYER_BUDGETS:
            # LAYER_BUDGETS stores Total Budget (History + Window)
            # max_capacity_prompt here represents History Budget
            total_budget = LAYER_BUDGETS[self.layer_idx]
            max_capacity_prompt = max(0, total_budget - self.window_size)
            # print(f"Layer {self.layer_idx}: RankKV override budget to {total_budget} (History: {max_capacity_prompt})")

        # print(f"PyramidKV max_capacity_prompt {max_capacity_prompt}")
        if q_len < self.max_capacity_prompt and self.layer_idx not in LAYER_BUDGETS:
            return key_states, value_states
            ...
            target_capacity = max_capacity_prompt
            if target_capacity > attn_cache.size(-1):
                target_capacity = attn_cache.size(-1)
                
            indices = attn_cache.topk(target_capacity, dim=-1).indices
            ...
            if max_capacity_prompt > attn_cache.size(-1):
                max_capacity_prompt = attn_cache.size(-1)
